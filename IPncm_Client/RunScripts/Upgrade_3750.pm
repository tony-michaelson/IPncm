...
my %known_types = (
	'c3750-ipservicesk9-mz.122-55.SE9.bin' => [
		'122-55.SE9',
		13009047,
		'18a3b07f66da1a03b4a93fb4c99d4989',
	], 
	'c3750-ipservicesk9-mz.122-55.SE10.bin' => [
		'122-55.SE10',
		13011760,
		'2d00a9768d61c0a947c9ccd96ccaa83f',
	],
	'c3750-ipservicesk9-mz.122-55.SE11.bin' => [
		'122-55.SE11',
		13013902,
		'c7d5b7edae840dbc7f39517dba5230f5',
	]
);
...
sub run  {
	...
	my $ios_md5_checksum = $known_types{$upgrade_file}->[2];
	if (!$ios_md5_checksum)  {
		fifo_log("$hostname", "exit", "No IOS MD5 Checksum Provided");
		$conn->log_error("No IOS MD5 Checksum Provided");
		return;
	}
	
	if (run_pre_installation($this, $conn, $hostname, $upgrade_file, 
			$upgrade_mode, $ios_md5_checksum, $queue_email, $email_subject, $upgrade_version, $upgrade_size) && $upgrade_mode)  {
...
sub run_pre_installation  {
	my ($this, $conn, $hostname, $upgrade_file, $upgrade_mode, $md5_checksum, $queue_email, $email_subject, $upgrade_version, 
			$upgrade_size) = @_;
	...
	if ($show_flash !~ /$upgrade_file/)  {
		...
		if ($cur_space <= $upgrade_size)  {
			fifo_log("$hostname", "Not enough space on flash: $cur_space", "$show_flash");
			while ($show_flash =~ /(\S+\.bin)/g)  {
				my $delete_flash = $conn->send_host("delete flash:" . $1);
				fifo_log("$hostname", "delete flash:" . $1, $delete_flash); 
			}
			# refresh flash information
			$show_flash = $conn->send_host("dir flash:");
			while ($show_flash =~ /(c3750\S+)/g)  {
				my $delete_flash = $conn->send_host("delete /recursive /force flash:/" . $1);
				fifo_log("$hostname", "delete /recursive /force flash:/" . $1, $delete_flash); 
			}
			$show_flash = $conn->send_host("dir flash:");
			$show_flash =~ /(\d+) bytes (?:available|free)/;
			$cur_space = $1;
			fifo_log("$hostname", "Found $cur_space bytes free on flash", "$show_flash");
		}
		if ($cur_space > $upgrade_size)  {
	...
	if ($verify_ios_flag) {
		fifo_log("$hostname", "Performing IOS Image File Verification", "verify /md5 flash:$upgrade_file");
		$verify_out = $conn->send_host("verify /md5 flash:$upgrade_file");
		fifo_log("$hostname", "verify /md5 flash:$upgrade_file", $verify_out);
		if ($verify_out =~ /$md5_checksum/i)  {
			fifo_log("$hostname", "Checksum verified: $md5_checksum", "$verify_out");
		} else {
			fifo_log("$hostname", "exit", "ABORTING INSTALL - IOS flash verification failed");
	...
	$show_switch = $conn->send_host("show switch");
	fifo_log("$hostname", "show switch", $show_switch);
...
sub run_post_installation  {
	...
	my $show_switch_new = $conn->send_host("show switch");
	fifo_log("$hostname", "show switch", "$show_switch_new");
	...
	$show_switch = line_filter($show_switch, qr/\w/);
	$show_switch_new = line_filter($show_switch_new, qr/\w/);
